...
describe('socketServer', () => {
    ...
    describe('openPairingChannel', () => {
        it('adds a pairing channel to state', () => {
            const state = State();
            const socket = { id: 'abc123' } as unknown as Socket;
    ...
    describe('sendPairingOffer', () => {
        it('emits a pairing offer', () => {
            const socket = { emit: vi.fn(), id: 'abc' } as unknown as Socket;
            const state = {
                someId: {
            ...
            const socket = {
                ...
                on: vi.fn((event, cb) => {
                    if (event === EventTypes.PairingRequest) {
                        cb({
                            pairingCode: 'unknownCode',
                            type: ClientTypes.Frontend,
                        });
                        cb({
                            pairingCode: 'knownCode',
    ...
    describe('startServer', () => {
        it('starts the server and logs output', () => {
            ...
            vi.spyOn(socketModule, 'createHttpServer').mockReturnValue(
                mockHttpServer as any
            );
            vi.spyOn(socketModule, 'createSocketServer').mockReturnValue(
            ...
            expect(logSpy).toHaveBeenCalledWith(
                expect.stringContaining('Socket.IO server listening')
            );
        });
    });
});
